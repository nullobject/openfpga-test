...
class DebugLayer(numCols: Int, numRows: Int) extends Module {
  ...
  val col = delta.x(log2Up(GPU.TILE_WIDTH) + log2Up(numCols) - 1, log2Up(GPU.TILE_WIDTH))
  val xOffset = delta.x(log2Up(GPU.TILE_WIDTH) - 1, 0)
  val yOffset = delta.y(log2Up(GPU.TILE_HEIGHT) - 1, 0)
